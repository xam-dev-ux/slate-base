...
export function useFundSummary(fund: Address | undefined) {
  ...
  const { data, isLoading, refetch } = useReadContracts({
    contracts: [
      ...
      { address: fund, abi: slateFundAbi, functionName: "maxDepositPerWallet" },
      { address: fund, abi: slateFundAbi, functionName: "maxTotalDeposits" },
  ...
  return {
    ...
    maxDepositPerWallet: data?.[7]?.result as bigint | undefined,
    maxTotalDeposits: data?.[8]?.result as bigint | undefined,
